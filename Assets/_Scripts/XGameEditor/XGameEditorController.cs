...
public class XGameEditorController : XGameController {
    ...
    public void InitTriggers(XGameEditorModel editor) {
        ...
        for (int i = 0; i < 10; i++) {
            XGameTrigger trigger = XGame.Resolve<XGameTriggerController>().CreateTrigger("trigger" + i, new List<XGameEvent> { new XGameEvent(XGameEventType.Character_Created) }, new List<XGameCondition> { condition, condition2 }, new List<XGameAction> { action });
